import { Box, Image, Text, List, ListItem, Flex, Heading } from "@chakra-ui/react";
...
export const UserList: React.FC<UserListProps> = ({ users, title, emptyMessage }) => {
  ...
  return (
    <Box m={4}>
      ...
      {
        users.length === 0 ? <Text>{emptyMessage}</Text> :
      <List spacing={3}>
        {users.map((user) => (
